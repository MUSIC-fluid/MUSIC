...
main(int argc, char *argv[])
{
  ...
  if (DATA.whichEOS==0)
    ...
    {
      if (rank == 0)
	fprintf(stderr,"Using lattice EOS from Huovinen/Petreczky with partial chemical equilibrium (PCE) chem. f.o. at 150 MeV\n");
      eos->init_eos3();
    }
  else if (DATA.whichEOS==4)
    {
      if (rank == 0)
	fprintf(stderr,"Using lattice EOS from Huovinen/Petreczky with partial chemical equilibrium (PCE) chem. f.o. at 155 MeV\n");
      eos->init_eos4();
    }
  else if (DATA.whichEOS==5)
    {
      if (rank == 0)
	fprintf(stderr,"Using lattice EOS from Huovinen/Petreczky with partial chemical equilibrium (PCE) chem. f.o. at 160 MeV\n");
      eos->init_eos5();
    }
  else if (DATA.whichEOS==6)
    {
      if (rank == 0)
	fprintf(stderr,"Using lattice EOS from Huovinen/Petreczky with partial chemical equilibrium (PCE) chem. f.o. at 165 MeV\n");
      eos->init_eos6();
    }
  else 
    {
      if (rank == 0)
	fprintf(stderr,"No EOS for whichEOS = %d, use EOS_to_use = 0 (ideal gas) 1 (AZHYDRO EOS-Q), 2 (s95p-v1), 3 (s95p-PCE150-v1), 4 (s95p-PCE155-v1), 5 (s95p-PCE160-v1), 6 (s95p-PCE165-v1) \n",DATA.whichEOS);
      exit(1);
    }
...
void ReadInData(InitData *DATA, char *file)
{
 ...
 DATA->turn_on_conductivity = util->IFind(file, "Include_Heat_Conductivity_Yes_1_No_0");
 ...
 DATA->t_conductivity_to_s = util->DFind(file, "TtimesConductivity_to_S_ratio");

//Following Hosoya and Kajantie
// fprintf(stderr, "T times Kappa is set to 0.3 times Eta.\n");
// DATA->t_conductivity_to_s = 0.3*(DATA->shear_to_s);
 
 DATA->tau_B = util->DFind(file, "Minimum_Heat_relaxation_time_tau_B");
