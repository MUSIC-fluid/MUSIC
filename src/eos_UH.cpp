...
EOS_UH::~EOS_UH() {
    ...
    for (int itable = 0; itable < ntables; itable++) {
        Util::mtx_free(mu_B_tb[itable], nb_length[itable], e_length[itable]);
    }
    delete[] mu_B_tb;
}
...
void EOS_UH::initialize_eos() {
    ...
    music_message.info(
        "Using lattice EOS at finite muB from the UH Collaboration");
    ...
    for (int itable = 0; itable < ntables; itable++) {
        ...
        std::ifstream eos_T(
            path + "UH_eos_T_" + eos_file_string_array[itable] + ".dat");
        std::ifstream eos_mub(
            path + "UH_eos_muB_" + eos_file_string_array[itable] + ".dat");
        ...
        ss >> dummy >> dummy >> e_bounds[itable] >> dummy >> e_spacing[itable]
            >> dummy >> e_length[itable] >> dummy >> nb_bounds[itable] >> dummy
            >> nb_spacing[itable] >> dummy >> nb_length[itable];
        ...
        temperature_tb[itable] =
            Util::mtx_malloc(nb_length[itable], e_length[itable]);
        mu_B_tb[itable] = Util::mtx_malloc(nb_length[itable], e_length[itable]);
...
double EOS_UH::p_e_func(double e, double rhob, double rhoq, double rhos) const {
    return (get_dpOverde3(e, rhob));
}

double EOS_UH::p_rho_func(
...
//! input local energy density eps [1/fm^4] and rhob [1/fm^3]
double EOS_UH::get_muB(double e, double rhob, double rhoq, double rhos) const {
    int table_idx = get_table_idx(e);
...
double EOS_UH::get_s2e(double s, double rhob, double rhoq, double rhos) const {
