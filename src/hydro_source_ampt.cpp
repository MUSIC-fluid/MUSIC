...
HydroSourceAMPT::HydroSourceAMPT(const InitData &DATA_in) : DATA(DATA_in) {
...
HydroSourceAMPT::~HydroSourceAMPT() { parton_list.clear(); }
...
void HydroSourceAMPT::read_in_AMPT_partons() {
    ...
    for (int ipart = 0; ipart < n_partons; ipart++) {
        ...
        text_stream >> pid >> new_parton->px >> new_parton->py >> pz_local
            >> new_parton->mass >> new_parton->x >> new_parton->y >> z_local
            >> t_local;
        ...
        new_parton->E = sqrt(
            new_parton->mass * new_parton->mass
            + new_parton->px * new_parton->px + new_parton->py * new_parton->py
            + pz_local * pz_local);
        new_parton->tau = sqrt(t_local * t_local - z_local * z_local);
        new_parton->eta_s =
            0.5
            * log((t_local + z_local) / (t_local - z_local + Util::small_eps));
        new_parton->rapidity =
            0.5 * log((new_parton->E + pz_local) / (new_parton->E - pz_local));
        double u_perp =
            (sqrt(
...
void HydroSourceAMPT::prepare_list_for_current_tau_frame(
    ...
    music_message << "hydro_source: tau = " << tau_local
                  << " number of source: " << parton_list_current_tau.size();
    music_message.flush("info");
}
...
void HydroSourceAMPT::get_hydro_energy_source(
    ...
    if (tau_dis_max < n_sigma_skip * sigma_tau) {
        ...
        double prefactor =
            norm * prefactor_tau * prefactor_prep * prefactor_etas;
        j_mu[0] *= prefactor;
        j_mu[1] *= prefactor;
        j_mu[2] *= prefactor;
        j_mu[3] *= prefactor;
...
double HydroSourceAMPT::get_hydro_rhob_source(
    ...
    if (tau_dis_max < n_sigma_skip * sigma_tau) {
        for (auto &it : parton_list_current_tau) {
            ...
            double f_smear = exp_tau * exp_xperp * exp_eta_s;
            double y_dump =
                ((1. - parton_quench_factor) * it->rapidity
                 + parton_quench_factor * y_long_flow);
            double y_dump_perp =
                ((1. - parton_quench_factor) * it->rapidity_perp
