...
EOS_neos::~EOS_neos() {
    ...
    for (int itable = 0; itable < ntables; itable++) {
        ...
        if (get_flag_muS()) {
            Util::mtx_free(
                mu_S_tb[itable], nb_length[itable], e_length[itable]);
        }
        if (get_flag_muQ()) {
            Util::mtx_free(
                mu_Q_tb[itable], nb_length[itable], e_length[itable]);
        }
    }
    if (ntables > 0) {
        delete[] mu_B_tb;
...
void EOS_neos::initialize_eos() {
    ...
    if (eos_id == 10) {
        ...
        std::copy(
            std::begin(string_tmp), std::end(string_tmp),
    ...
    for (int itable = 0; itable < ntables; itable++) {
        std::ifstream eos_p(
            path + "neos" + eos_file_string_array[itable] + "_p.dat");
        if (!eos_p.is_open()) {
        ...
        std::ifstream eos_T(
            path + "neos" + eos_file_string_array[itable] + "_t.dat");
        std::ifstream eos_mub(
            path + "neos" + eos_file_string_array[itable] + "_mub.dat");
        std::ifstream eos_muS;
        std::ifstream eos_muQ;
        if (flag_muS) {
            eos_muS.open(
                path + "neos" + eos_file_string_array[itable] + "_mus.dat");
            if (!eos_muS.is_open()) {
                music_message << "Can not open EOS files: "
                              << (path + "neos" + eos_file_string_array[itable]
                                  + "_mus.dat");
        ...
        if (flag_muQ) {
            eos_muQ.open(
                path + "neos" + eos_file_string_array[itable] + "_muq.dat");
            if (!eos_muQ.is_open()) {
                music_message << "Can not open EOS files: "
                              << (path + "neos" + eos_file_string_array[itable]
                                  + "_muc.dat");
        ...
        eos_p >> nb_spacing[itable] >> e_spacing[itable] >> N_rhob >> N_e;
        ...
        for (int j = 0; j < e_length[itable]; j++) {
            for (int i = 0; i < nb_length[itable]; i++) {
                ...
                temperature_tb[itable][i][j] =
                    pow(temperature_tb[itable][i][j], 5.);  // 1/fm^5
