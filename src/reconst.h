...
class Reconst {
 ...
 public:
    ...
    ReconstCell ReconstIt_shell(double tau, std::array<double,5> &uq, Cell &grid_pt, int rk_flag);

    int ReconstIt(ReconstCell &grid_p, double tau, std::array<double,5> &uq, Cell *grid_pt,
                  int rk_flag);
    double GuessEps(double T00, double K00, double cs2);
    
    void revert_grid(ReconstCell &grid_current, Cell *grid_prev, int rk_flag);

    int ReconstIt_velocity_iteration(ReconstCell &grid_p, double tau,
                                     std::array<double,5> &uq, Cell *grid_pt, int rk_flag);
    ...
    void regulate_grid(ReconstCell &grid_cell, double elocal, int rk_flag);
};
