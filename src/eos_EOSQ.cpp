...
void EOS_eosQ::initialize_eos() {
    ...
    for (int itable = 0; itable < ntables; itable++) {
        std::ifstream eos_p(
            envPath + "/EOS/EOS-Q/aa" + eos_file_string_array[itable]
            + "_p.dat");
        std::ifstream eos_T(
            envPath + "/EOS/EOS-Q/aa" + eos_file_string_array[itable]
            + "_t.dat");
...
double EOS_eosQ::get_temperature(
    double e, double rhob, double rhoq, double rhos) const {
