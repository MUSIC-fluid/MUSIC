...
void Freeze::ReadSpectra_pseudo(InitData *DATA, int full, int verbose) {
    ...
    while (fscanf(
               p_file, "%d %lf %d %lf %lf %d %d ", &number, &etamax, &ietamax,
               &ptmin, &ptmax, &iptmax, &iphimax)
           == 7) {
        count++;
        ...
        for (int ipt = 0; ipt < iptmax; ipt++) {
            particleList[ip].pt[ipt] =
                (ptmin
                 + (ptmax - ptmin) * pow(static_cast<double>(ipt), 2.)
...
void Freeze::ComputeParticleSpectrum_pseudo_improved(
    ...
    music_message << "Doing " << j << ": " << particleList[j].name << "("
...
#pragma omp for
            for (int icell = 0; icell < NCells; icell++) {
                ...
                double prefactor_shear =
                    1. / (2. * eps_plus_P_over_T * T * T * T) * hbarc;
                // fm^4/GeV^2
                ...
                for (int ipt = 0; ipt < iptmax; ipt++) {
                    ...
                    if (fabs(y - eta_s) < y_minus_eta_cut) {
                        ...
                        double ptau = mt
                                      * (cosh_y_local * cosh_eta_s
                                         - sinh_y_local * sinh_eta_s);
                        double peta = mt
                                      * (sinh_y_local * cosh_eta_s
                        ...
                        for (int iphi = 0; iphi < iphimax; iphi++) {
                            ...
                            double pdSigma =
                                tau
                                * (ptau * sigma_mu[0] + px * sigma_mu[1]
                                   + py * sigma_mu[2]
                                   + peta / tau * sigma_mu[3]);
                            double E =
                                (ptau * u_flow[0] - px * u_flow[1]
                            ...
                            if (flag_shear_deltaf == 1) {
                                Wfactor =
                                    (ptau * W00 * ptau - 2. * ptau * W01 * px
                                     - 2. * ptau * W02 * py
                                     - 2. * ptau * W03 * peta + px * W11 * px
                                     + 2. * px * W12 * py + 2. * px * W13 * peta
                                     + py * W22 * py + 2. * py * W23 * peta
                                     + peta * W33 * peta);
                                delta_f_shear =
                                    (f * (1. - sign * f) * prefactor_shear
                                     * Wfactor);
                            ...
                            if (flag_qmu_deltaf == 1) {
                                // p^\mu q_\mu
                                qmufactor =
                                    (ptau * qmu_0 - px * qmu_1 - py * qmu_2
                                     - peta * qmu_3);
                                if (DATA->deltaf_14moments == 0) {
                                    delta_f_qmu =
                                        (f * (1. - sign * f)
                                         * (prefactor_qmu - baryon / E)
                                         * qmufactor / deltaf_qmu_coeff);
                                } else {
                                    delta_f_qmu =
                                        (f * (1. - sign * f)
                                         * (baryon * deltaf_qmu_coeff_14mom_DV
                                            + 2. * deltaf_qmu_coeff_14mom_BV
                                                  * E)
                                         * qmufactor);
                                }
                            }
                            ...
                            if (sum > 10000) {
                                music_message
                                    << "sum>10000 in summation. sum = " << sum
                                    << ", f=" << f
                                    << ", deltaf=" << delta_f_shear
                                    << ", pdSigma=" << pdSigma << ", T=" << T
                                    << ", E=" << E << ", mu=" << mu;
...
#pragma omp critical
        ...
        delete[] cosh_y;
        delete[] sinh_y;
        for (int ipt = 0; ipt < iptmax; ipt++) delete[] temp_sum[ipt];
        delete[] temp_sum;
    }
                ...
                for (int ipt = 0; ipt < iptmax; ipt++) {
                    ...
                    for (int iphi = 0; iphi < iphimax; iphi++) {
                        ...
                        double pdSigma =
                            tau
                            * (ptau * sigma_mu[0] + px * sigma_mu[1]
                               + py * sigma_mu[2] + peta * sigma_mu[3] / tau);
                        double E =
                            (ptau * u_flow[0] - px * u_flow[1] - py * u_flow[2]
                             - peta * u_flow[3]);
                        // this is the equilibrium f, f_0:
                        double f = 1. / (exp(1. / T * (E - mu)) + sign);
                        ...
                        if (flag_shear_deltaf == 1) {
                            Wfactor =
                                (ptau * W00 * ptau - 2. * ptau * W01 * px
                                 - 2. * ptau * W02 * py - 2. * ptau * W03 * peta
                                 + px * W11 * px + 2. * px * W12 * py
                                 + 2. * px * W13 * peta + py * W22 * py
                                 + 2. * py * W23 * peta + peta * W33 * peta);
                            delta_f_shear =
                                (f * (1. - sign * f) * prefactor_shear
                                 * Wfactor);
                            if (DATA->include_deltaf == 2) {
...
void Freeze::OutputFullParticleSpectrum_pseudo(
    ...
    fprintf(
        d_file, "%d %e %d %e %e %d %d \n", number, DATA->max_pseudorapidity,
        DATA->pseudo_steps + 1, DATA->min_pt, DATA->max_pt, DATA->pt_steps + 1,
        DATA->phi_steps);
    ...
    for (int iy = 0; iy <= DATA->pseudo_steps; iy++) {
        for (int ipt = 0; ipt <= DATA->pt_steps; ipt++) {
            for (int iphi = 0; iphi < DATA->phi_steps; iphi++) {
                fprintf(
                    s_file, "%e ", particleList[j].dNdydptdphi[iy][ipt][iphi]);
            }
            fprintf(s_file, "\n");
        }
    }
...
void Freeze::compute_thermal_spectra(
    int particleSpectrumNumber, InitData *DATA) {
    double mass_tol = 1e-3;
    double mu_tol = 1e-3;

    // clean up
    system_status_ = system(
        "rm yptphiSpectra.dat yptphiSpectra?.dat "
    ...
    if (particleSpectrumNumber == 0) {
        ...
        for (int i = 1; i < particleMax_copy; i++) {
            ...
            for (int part = 1; part < i; part++) {
                double mass_diff =
                    fabs(particleList[i].mass - particleList[part].mass);
                double mu_diff = fabs(
                ...
                if (mass_diff < mass_tol && mu_diff < mu_tol
                    ...
                    fprintf(
                        d_file, "%d %e %d %e %e %d %d \n", number, etamax,
                        ietamax, ptmin, ptmax, iptmax, iphimax);
                    for (int ieta = 0; ieta < ietamax; ieta++) {
                        for (int ipt = 0; ipt < iptmax; ipt++) {
...
void Freeze::CooperFrye_pseudo(
    int particleSpectrumNumber, int mode, InitData *DATA, EOS *eos) {
...
double Freeze::PseudoRap(double y, double pt, double m) {
    double eta =
        acosh(2 * m * m / pt / pt * sinh(y) * sinh(y) + cosh(2 * y)) / 2.;
    if (y < 0) eta *= -1.;
    return eta;
}
...
double Freeze::Rap(double eta, double pt, double m) {
    double y =
        log((sqrt(m * m + pt * pt * cosh(eta) * cosh(eta)) + pt * sinh(eta))
            / sqrt(m * m + pt * pt));
    return y;
}
...
void Freeze::rapidity_integrated_flow(
    ...
    if (minrap < testmin) {
        music_message
            << "Error: called out of range rapidity in rap_integrated_flow, "
    ...
    if (yflag) {
        if (DATA->pseudofreeze == 1)
            testmax =
                Rap(particleList[j].y[neta - 1], particleList[j].pt[0], m);
        else
...
void Freeze::pt_and_rapidity_integrated_flow(
    InitData *DATA, int number, double minpt, double maxpt, int yflag,
    double minrap, double maxrap, double vn[nharmonics][2]) {
    int j = partid[MHALF + number];
    int npt = particleList[j].npt;

    if (minpt < particleList[j].pt[0]) {
        music_message << "Error: called out of range pt in "
                      << "pt_and_rapidity_integrated_flow, " << minpt
                      << " < minimum " << particleList[j].pt[0];
        music_message.flush("error");
        exit(1);
    }
    if (maxpt > particleList[j].pt[npt - 1]) {
        music_message << "Error: called out of range pt in "
                      << "pt_and_rapidity_integrated_flow, " << maxpt
                      << " > maximum " << particleList[j].pt[npt - 1];
        music_message.flush("error");
        exit(1);
    }
    if (minpt > maxpt) {
...
void Freeze::OutputDifferentialFlowAtMidrapidity(
    ...
    for (int ipt = 0; ipt < npt; ipt++) {
        ...
        pt_and_rapidity_integrated_flow(
            DATA, number, pt, pt, 1, eta, eta, vn);  // in rapidity

        // Output result
        outfilevn << pt;
        outfilevn << "  " << vn[0][0] / pt / (2 * M_PI);
        for (int i = 1; i < nharmonics; i++)
            for (int k = 0; k < 2; k++) outfilevn << "  " << vn[i][k];
        outfilevn << endl;

        pt_and_rapidity_integrated_flow(
            DATA, number, pt, pt, 0, eta, eta, vn);  // pseudo-rapidity
        outfilevn2 << pt;
        outfilevn2 << "  " << vn[0][0] / pt / (2 * M_PI);
        for (int i = 1; i < nharmonics; i++)
            for (int k = 0; k < 2; k++) outfilevn2 << "  " << vn[i][k];
        outfilevn2 << endl;
    }
...
void Freeze::OutputDifferentialFlowNearMidrapidity(
    ...
    stringstream fname;
    if (full) {
        fname << "./outputs/Fvnpt-" << number << "_y_" << y_min << "_" << y_max
              << ".dat";
    } else {
        fname << "./outputs/vnpt-" << number << "_y_" << y_min << "_" << y_max
              << ".dat";
    }

    stringstream fname2;
    if (full) {
        fname2 << "./outputs/Fvnpt-" << number << "_eta_" << eta_min << "_"
               << eta_max << ".dat";
    } else {
        fname2 << "./outputs/vnpt-" << number << "_eta_" << eta_min << "_"
               << eta_max << ".dat";
    }
    ...
    for (int ipt = 0; ipt < npt; ipt++) {
        ...
        pt_and_rapidity_integrated_flow(
            DATA, number, pt, pt, 1, y_min, y_max, vn);  // rapidity
        ...
        // pseudo-rapidity
        pt_and_rapidity_integrated_flow(
            DATA, number, pt, pt, 0, eta_min, eta_max, vn);
        // Output result
        outfilevn2 << pt;
...
void Freeze::OutputIntegratedFlow_vs_y(
    ...
    music_message << "Calculating flow integrated between " << pT_min
                  << " < p_T < " << pT_max
                  << " vs. pseudorapidity for particle " << number;
    music_message.flush("info");

    // Set output file name
    stringstream fname;
    if (full) {
        fname << "./outputs/Fvn_y-" << number << "_pT_" << pT_min << "_"
              << pT_max << ".dat";
    } else {
        fname << "./outputs/vn_y-" << number << "_pT_" << pT_min << "_"
              << pT_max << ".dat";
    }

    stringstream fname2;
    if (full) {
        fname2 << "./outputs/Fvn_eta-" << number << "_pT_" << pT_min << "_"
               << pT_max << ".dat";
    } else {
        fname2 << "./outputs/vn_eta-" << number << "_pT_" << pT_min << "_"
               << pT_max << ".dat";
    }
    ...
    outfilevn2
        << "#eta  dN/deta dummy  v1cos  v1sin  v2cos  v2sin  v3cos  v3sin  "
        << "v4cos  v4sin  v5cos  v5sin  v6cos  v6sin  v7cos  v7sin\n";
    ...
    for (int irap = 0; irap < nrap; irap++) {
        ...
        // rapidity
        pt_and_rapidity_integrated_flow(
            DATA, number, pT_min, pT_max, 1, y_local, y_local, vn);
        ...
        // pseudo-rapidity
        pt_and_rapidity_integrated_flow(
            DATA, number, pT_min, pT_max, 0, eta_local, eta_local, vn);
...
void Freeze::Output_charged_IntegratedFlow(
    InitData *DATA, double pT_min, double pT_max, double eta_min,
    double eta_max) {
    music_message << "Calculating charged hadron flow integrated between "
                  << pT_min << " < p_T < " << pT_max << " and " << eta_min
                  << " < eta < " << eta_max << "...";
    music_message.flush("info");

    // Set output file name
    stringstream fname;
    fname << "./outputs/vnch_pT_" << pT_min << "_" << pT_max << "_eta_"
    ...
    outfile << std::scientific << std::setw(18) << std::setprecision(8) << dNch
            << "  ";
...
void Freeze::Output_charged_hadrons_eta_differential_spectra(
    ...
    for (int ieta = 0; ieta < nrap; ieta++) {
        ...
        for (int iorder = 1; iorder < max_flow_order; iorder++) {
            double *vn_temp = new double[2];
            get_vn_ch(
                DATA, pT_min, pT_max, 0, eta_local, eta_local, iorder, vn_temp);
            outfile << vn_temp[0] << "  " << vn_temp[1] << "  ";
            delete[] vn_temp;
        }
        outfile << endl;
...
void Freeze::Output_charged_hadrons_pT_differential_spectra(
    ...
    music_message
        << "Collecting charged hadrons dN/detapTdpT and vn(pT) vs pT...";
    music_message.flush("info");
    ...
    for (int ipT = 0; ipT < npT; ipT++) {
        ...
        for (int iorder = 1; iorder < max_flow_order; iorder++) {
            double *vn_temp = new double[2];
            get_vn_ch(
                DATA, pT_local, pT_local, 0, eta_min, eta_max, iorder, vn_temp);
            outfile << vn_temp[0] << "  " << vn_temp[1] << "  ";
            delete[] vn_temp;
        }
        outfile << endl;
...
double Freeze::get_N(
    ...
    double vn[nharmonics][2];
    pt_and_rapidity_integrated_flow(
        DATA, number, minpt, maxpt, yflag, minrap, maxrap, vn);
    return (vn[0][0]);
}
...
double Freeze::get_Nch(
    InitData *DATA, double minpt, double maxpt, int yflag, double minrap,
    double maxrap) {
    double N = 0;
...
double Freeze::get_vn_ch(
    InitData *DATA, double minpt, double maxpt, int yflag, double minrap,
    double maxrap, int n, double *vn_results) {
    if (n > nharmonics - 1) {
...
void Freeze::weighted_v1(
    InitData *DATA, int number, double minpt, double maxpt, int yflag,
    double minrap, double maxrap, double vn[2], int ch) {
    int j = partid[MHALF + number];
    int npt = particleList[j].npt;
    if (minpt < particleList[j].pt[0]) {
        music_message << "Error: called out of range pt in get_meanpt, "
...
double Freeze::get_weighted_v1(
    InitData *DATA, int number, double minpt, double maxpt, int yflag,
    double minrap, double maxrap, int ch) {
    double vn[2];
    weighted_v1(DATA, number, minpt, maxpt, yflag, minrap, maxrap, vn, ch);
    return vn[0];
}

double Freeze::get_weighted_psi1(
    InitData *DATA, int number, double minpt, double maxpt, int yflag,
    double minrap, double maxrap, int ch) {
    double vn[2];
    weighted_v1(DATA, number, minpt, maxpt, yflag, minrap, maxrap, vn, ch);
