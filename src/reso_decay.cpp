...
// generated by the hydrodynamical code azhydro0p2.  The majority of the code
// was developed by Josef Sollfrank and Peter Kolb.  Additional work and
// comments were added by Evan Frodermann, September 2005. Please refer to the
// papers J. Sollfrank, P. Koch, and U. Heinz, Phys. Lett B 252 (1990) and J.
// Sollfrank, P. Koch, and U. Heinz, Z. Phys. C 52 (1991) for a description of
// the formalism utilized in this program.
...
double Freeze::Edndp3(double yr, double ptr, double phirin, int res_num) {
    ...
    double val1 = Util::lin_int(
        particleList[pn].pt[npt - 1], particleList[pn].pt[npt], f1, f2, ptr);
    ...
    double val = Util::lin_int(
        particleList[pn].y[ny - 1], particleList[pn].y[ny], val1, val2, yr);
...
double Freeze::dnpir2N(double phi, void *para1) {
    ...
    cphiR = -jac
            * (para->p0 * para->sinth * cos(phi + para->phi)
               - para->pt * cos(para->phi))
            / (sume * ptR);
    sphiR = -jac
            * (para->p0 * para->sinth * sin(phi + para->phi)
               - para->pt * sin(para->phi))
            / (sume * ptR);
    ...
    phiR = acos(cphiR);
    if (sphiR < 0.0) phiR = 2.0 * M_PI - phiR;
...
double Freeze::norm3int(
    double x, void *paranorm)  // this computes "Q(m_R,m_1,m_2,m_3)"
{
    nblock *tmp = (nblock *)paranorm;
    double res =
        sqrt((tmp->a - x) * (tmp->b - x) * (x - tmp->c) * (x - tmp->d)) / x;
    return res;
}
...
double Freeze::dnpir1N(double costh, void *para1) {
    ...
    r = gauss(
        PTN2, &Freeze::dnpir2N, 0.0, 2.0 * M_PI,
        para);  // Integrates the "dnpir2N" kernel over phi using gaussian
                // integration
    return r;
}
...
double Freeze::dn2ptN(double w2, void *para1) {
    pblockN *para = (pblockN *)para1;
    para->e0 = (para->mr * para->mr + para->m1 * para->m1 - w2)
               / (2 * para->mr);  // particle one energy in resonance rest frame
    para->p0 = sqrt(
        para->e0 * para->e0
        - para->m1 * para->m1);  // particle one absolute value of three
                                 // momentum on resonance rest frame
    return gauss(
        PTN1, &Freeze::dnpir1N, -1.0, 1.0,
        para);  // Integrate the "dnpir1N" kernel over cos(theta) using gaussian
                // integration
}

double Freeze::dn3ptN(
    double x, void *para1)  // The integration kernel for "W" in 3-body decays.
                            // x=invariant mass of other particles squared
{
    pblockN *para = (pblockN *)para1;
    double e0 =
        (para->mr * para->mr + para->m1 * para->m1 - x) / (2 * para->mr);
...

{
    ...
    res2 =
        norm2
        * dn2ptN(m2 * m2, &para);  // Calls the integration routines for 2-body
    if (res2 < 0.) res2 = 0.;
 ...
 * transverse momentum spectrum in GeV^-2 from pions out of resonances
 *********************************************************************/
double Freeze::Edndp3_3bodyN(
    double y, double pt, double phi, double m1, double m2, double m3, double mr,
    double norm3, int res_num)
/* in units of GeV^-2,includes phasespace and volume,
   does not include degeneracy factors  */
{
    double mt = sqrt(pt * pt + m1 * m1);
    ...
    wmin = (m2 + m3) * (m2 + m3);
    wmax = (mr - m1) * (mr - m1);
    res3 = 2.0 * norm3 * gauss(PTS4, &Freeze::dn3ptN, wmin, wmax, &para)
           / mr;  // Integrates "W" using gaussian
    if (res3 < 0.) res3 = 0.;
    return res3;
}
                ...
                for (int l = 0; l < npt; l++)
                    for (int i = 0; i < nphi; i++) {
                        ...
                        double spectrum = Edndp3_2bodyN(
                            y, particleList[pn].pt[l], phi, m1, m2, mr,
...
#pragma omp parallel for collapse(3)
        ...
        case 3:  // 3-body decay
        {
            ...
            norm3 = mr * mr
                    / (2 * M_PI
                       * gauss(
                           PTS3, &Freeze::norm3int, paranorm.c, paranorm.b,
                           &paranorm));
                ...
                for (int l = 0; l < npt; l++)
                    for (int i = 0; i < nphi; i++) {
                        ...
                        double spectrum = Edndp3_3bodyN(
                            y, particleList[pn].pt[l], phi, m1, m2, m3, mr,
                            norm3, particleList[pnR].number);
                        if (std::isnan(spectrum)) {
                            fprintf(
                                stderr, "3 number=%d\n",
                                particleList[pnR].number);
...
#pragma omp parallel for collapse(3)
        ...
        case 4:  // 4-body decay (rare and low contribution)
        {
            ...
            mr = particleList[pnR].mass;
            m3 = 0.5
                 * (particleList[pn3].mass + particleList[pn4].mass + mr - m1
                    - m2);
            paranorm.a = (mr + m1) * (mr + m1);
            paranorm.b = (mr - m1) * (mr - m1);
            paranorm.c = (m2 + m3) * (m2 + m3);
...
void Freeze::cal_reso_decays(int maxpart, int maxdecay, int bound) {
    ...
    for (int i = maxpart - 1; i > pn - 1; i--) {
        ...
        music_message << "Calculating the decays with " << particleList[i].name;
        music_message.flush("info");
