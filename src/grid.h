...
class GridT {
  private:
    ...
    T& get(int x, int y, int eta) { return grid[Nx * (Ny * eta + y) + x]; }
  ...
  public:
    ...
    const T& getHalo(int x, int y, int eta) const { return getHalo(x, y, eta); }
...
#define NLAMBDAS                                                    \
    [&](Cell_small & c, const Cell_small& p1, const Cell_small& p2, \
        const Cell_small& m1, const Cell_small& m2, const int direction)
