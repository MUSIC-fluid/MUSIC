...
bool Polygon::add_line(Line *l, int donotcheck) {
    ...
    if (donotcheck || Nlines == 0) {
        ...
        if (diff1 < eps || diff2 < eps) {
            // We always want that start point is connected to end so we flip
            // the start and end in the new line if needed
...
bool Polyhedron::add_polygon(Polygon *p, int donocheck) {
    ...
    if (donocheck || Npolygons == 0) {
        ...
        // Here we check if the new polygon is connected with the last polygon
        // in polyhedron
...
void Square::construct_lines(double E0) {
    ...
    // First we find the cut points and the points which are always outside of
    // the surface. Also find_outside() arranges cuts so that first two cuts
    // form a line as defined in the algorithm (in case there are 4 cuts)
    ends_of_edge(E0);
...
void Square::ends_of_edge(double E0) {
    ...
    if ((points[0][0] - E0) * (points[1][0] - E0) < 0) {
        cuts[Ncuts][0] =
            (points[0][0] - E0) / (points[0][0] - points[1][0]) * dx[x1];
        cuts[Ncuts][1] = 0;
        Ncuts++;
    ...
    if ((points[0][0] - E0) * (points[0][1] - E0) < 0) {
        cuts[Ncuts][0] = 0;
        cuts[Ncuts][1] =
            (points[0][0] - E0) / (points[0][0] - points[0][1]) * dx[x2];
        Ncuts++;
    ...
    if ((points[1][0] - E0) * (points[1][1] - E0) < 0) {
        cuts[Ncuts][0] = dx[x1];
        cuts[Ncuts][1] =
            (points[1][0] - E0) / (points[1][0] - points[1][1]) * dx[x2];
        Ncuts++;
    ...
    if ((points[0][1] - E0) * (points[1][1] - E0) < 0) {
        cuts[Ncuts][0] =
            (points[0][1] - E0) / (points[0][1] - points[1][1]) * dx[x1];
        cuts[Ncuts][1] = dx[x2];
        Ncuts++;
...
void Square::find_outside(double E0) {
    if (Ncuts == 4) {  // Here the surface is ambiguous
        ...
        if ((points[0][0] < E0 && Emid < E0)
            || (points[0][0] > E0 && Emid > E0)) {
            for (int i = 0; i < 2; i++) {
...
void Cube::construct_polygons(double value0) {
    ...
    if (ambiguous > 0) {
        // Surface is ambiguous, so let's connect the lines to polygons and see
        // how many polygons we have
        ...
        do {
            if (Nlines - used < 3) {
                cout << "Error: cannot construct polygon from " << Nlines - used
                     << " lines" << endl;
                exit(1);
            }
            ...
            for (int i = 0; i < Nlines; i++) {
                if (not_used[i]) {
                    ...
                    if (polygons[Npolygons].add_line(lines[i], 0)) {
                        not_used[i] = 0;
                        used++;
                        // if line is succesfully added we start the loop from
                        // the beginning
                        i = 0;
                    }
                }
 ...
 * surface is ambiguous and if it is not amibiguous we add all polygons to a
 * single polyhedron. If surface is ambiguous we need to connect the polygons
...
void Hypercube::construct_polyhedrons(double value0) {
    ...
    if (ambiguous > 0) {
        // Here surface might be ambiguous and we need to connect the polygons
        ...
        do {
            ...
            for (int i = 0; i < Npolygons; i++) {
                if (not_used[i]) {
                    // add_polygon returns true if the polygon is succesfully
                    // added
                    if (polyhedrons[Npolyhedrons].add_polygon(polygons[i], 0)) {
                        not_used[i] = 0;
                        used++;
                        // if polygon is succesfully added we start the loop
                        // from the beginning
                        i = 0;
                    }
                }
 ...
 * elements are not printed with this, but instead this is used to print the
 * triangles, which are used to construct the final elements.
 *
 ...
 * value [0][0] is at (0,0,0) and [1][1] is at (dx1,dx2).
 *
 */
 ...
 * value [0][0][0] is at (0,0,0) and [1][1][1] is at (dx1,dx2,dx3).
 *
 */
 ...
 * triangles which are found by the algorithm.
 *
 * @param [in] cube Values at the corners of the cube as a 3d table so that
 * value [0][0][0] is at (0,0,0) and [1][1][1] is at (dx1,dx2,dx3).
...
void Cornelius::surface_3d(double ***cube, double *pos, int do_print) {
    ...
    for (int i = 0; i < Nelements; i++) {
        ...
        // If we want to print the actual triangles, which are found, we must do
        // it here before polygons are removed from the memory.
 ...
 * value [0][0][0][0] is at (0,0,0,0) and [1][1][1][1] is at (dx1,dx2,dx3,dx4).
 *
 */
 ...
 * than four, first (4-dimension) elements are zero. Note that this function
 * allocates memory and the user must free it!
 *
 ...
 * four, first (4-dimension) elements are zero. This gives \sigma_\mu without
 * factors(sqrt(-g)) from the metric. Note that this function allocates memory
 * and the user must free it!
 *
 * @return Table with dimensions [number of elements][4] containing the normal
 *         vectors of the surface elements.
 *
 ...
 *
 * Returns the centroid vectors as a 2d table with the following number of
 * indices [number of elements][dimension of the problem]. Note that this
 * function allocates memory and the user must free it!
 *
 * @return Table with dimensions [number of elements][dimensions] containing the
 * centroid vectors of the surface elements.
 *
 */
 ...
 *
 * @return Table with dimensions [number of elements][dimensions] containing the
 * normal vectors of the surface elements.
 *
 */
 ...
 * Valid values are [0,number of elements in this cube].
 ...
 * factors (sqrt(-g)) from the metric.
 *
