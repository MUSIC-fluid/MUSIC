...
class EOS_4D : public EOS_base {
 private:
    ...
    int index(int i_T, int i_mub, int i_muq, int i_mus);
    //int Cfloor(double val);
    int check_index(int ind, int lim);
    std::vector<double> FourDLInterp(std::vector<double>& data, 
		    std::vector<double> TildeVar, bool compute_derivatives=false);
    std::vector<double> get_tilde_variables(double e, double nb, double nq, double ns);
 ...
 public:
    ...
    void initialize_eos();
    double get_temperature(double e, double rhob, double rhoq=0.0, double rhos=0.0);
    double get_muB        (double e, double rhob, double rhoq=0.0, double rhos=0.0);
    double get_muS        (double e, double rhob, double rhoq=0.0, double rhos=0.0);
    double get_muQ        (double e, double rhob, double rhoq=0.0, double rhos=0.0);
    double get_pressure   (double e, double rhob, double rhoq=0.0, double rhos=0.0);

    void get_pressure_with_gradients(double epsilon, double rhob, double &p, 
		    double &dpde, double &dpdrhob, double &cs2, 
		    double rhoq=0.0, double rhos=0.0);

    void get_pressure_with_gradients(double epsilon, double rhob, double &p, 
		    double &dpde, double &dpdrhob, 
		    double &dpdrhoq, double &dpdrhos, 
		    double &cs2, double rhoq=0.0, double rhos=0.0);
