...
class U_derivative {
 ...
 public:
    ...
    ~U_derivative();
    int MakedU(double tau, InitData *DATA, Cell ***arena, int rk_flag);

    //! this function returns the expansion rate on the grid
    double calculate_expansion_rate(double tau, Cell ***arena,
                                    int ieta, int ix, int iy, int rk_flag);

    //! this function returns Du^\mu
    void calculate_Du_supmu(double tau, Cell ***arena, int ieta, int ix,
                            int iy, int rk_flag, double *a);

    //! This funciton returns the velocity shear tensor sigma^\mu\nu
    void calculate_velocity_shear_tensor(double tau, Cell ***arena,
        int ieta, int ix, int iy, int rk_flag, double *a_local, double *sigma);
    int MakeDSpatial(double tau, InitData *DATA, Cell *grid_pt, int rk_flag);
