...
class HydroSourceBase {
  ...
  public:
    ...
    void set_source_tauStart_max(double tau_in) {
        source_tauStart_max_ = tau_in;
    }
    ...
    virtual double get_hydro_rhob_source(
        const double tau, const double x, const double y, const double eta_s,
    ...
    //! at a given point (tau, x, y, eta_s)
    virtual double get_hydro_rhoQ_source(
        const double tau, const double x, const double y, const double eta_s,
        const FlowVec &u_mu) const {
        return (0.0);
    }
    ...
    double get_hydro_rhob_source_before_tau(
        const double tau, const double x, const double y,
