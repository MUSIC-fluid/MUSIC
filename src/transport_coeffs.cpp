...
void TransportCoeffs::read_in_shear_from_file() {
    ...
    if (!shearFile) {
        std::cout << "Can not find the shear viscosity file: " << path
                  << std::endl;
        exit(1);
    }
    TArr_.resize(100);
...
void TransportCoeffs::read_in_bulk_from_file() {
    ...
    if (!bulkFile) {
        std::cout << "Can not find the bulk viscosity file: " << path
                  << std::endl;
        exit(1);
    }
    TArr_.resize(100);
...
double TransportCoeffs::get_eta_over_s(const double T, const double muB) const {
    ...
    if (shear_T_ == 0) {
        ...
        int Tidx =
            static_cast<int>((T_in_GeV - TArr_[0]) / (TArr_[1] - TArr_[0]));
        ...
        eta_over_s =
            (1. - Tfrac) * shearArr_[Tidx] + Tfrac * shearArr_[Tidx + 1];
    } else if (shear_T_ == 3) {
...
double TransportCoeffs::get_temperature_dependent_eta_over_s_default(
    ...
    if (Tfrac < 1.) {
        eta_over_s =
            (DATA.shear_to_s + 0.0594 * (1. - Tfrac)
             + 0.544 * (1. - Tfrac * Tfrac));
    } else {
        eta_over_s =
            (DATA.shear_to_s + 0.288 * (Tfrac - 1.)
...
double TransportCoeffs::get_temperature_dependent_eta_over_s_sims(
    ...
    if (T_in_GeV < T_kink_in_GeV) {
        eta_over_s =
            (eta_over_s_at_kink + low_T_slope * (T_in_GeV - T_kink_in_GeV));
    } else {
        eta_over_s =
            (eta_over_s_at_kink + high_T_slope * (T_in_GeV - T_kink_in_GeV));
    }
    eta_over_s = std::max(eta_over_s, eta_over_s_min);
    return (eta_over_s);
}

double TransportCoeffs::get_zeta_over_s(
    const double T, const double mu_B) const {
    ...
    if (bulk_T_ == 23) {
        ...
        Tfrac = std::max(0., std::min(1., Tfrac));
        zeta_over_s =
            (1. - Tfrac) * bulkArr_[Tidx] + Tfrac * bulkArr_[Tidx + 1];
    } else if (bulk_T_ == 10) {
        ...
        if (muB_in_GeV < 0.2) {
            peak_norm =
                (DATA.bulk_10_max
                 + (DATA.bulk_10_max_muB0p2 - DATA.bulk_10_max) / 0.2
                       * muB_in_GeV);
        } else if (muB_in_GeV < 0.4) {
            peak_norm =
                (DATA.bulk_10_max_muB0p2
                 + (DATA.bulk_10_max_muB0p4 - DATA.bulk_10_max_muB0p2) / 0.2
                       * (muB_in_GeV - 0.2));
        } else {
        ...
        const double Tpeak =
            (DATA.bulk_10_Tpeak
             + DATA.bulk_10_Tpeak_muBcurv * muB_in_GeV * muB_in_GeV);  // GeV
...
double TransportCoeffs::get_temperature_dependent_zeta_over_s_default(
    ...
    if (T_in_GeV < 0.995 * Ttr) {
        bulk =
            (lambda3 * exp((dummy - 1) / sigma3)
             + lambda4 * exp((dummy - 1) / sigma4) + 0.03);
    }
    if (T_in_GeV > 1.05 * Ttr) {
        bulk =
            (lambda1 * exp(-(dummy - 1) / sigma1)
...
double TransportCoeffs::getResummedCorrFactor(
    ...
    double r_combined = DATA.resumTransCoeffAlpha * (Rshear + Rbulk);
    if (r_combined < 1.0) {
