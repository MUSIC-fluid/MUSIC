...
HydroSourceStrings::HydroSourceStrings(InitData &DATA_in) : DATA(DATA_in) {
...
void HydroSourceStrings::read_in_QCD_strings_and_partons() {
    ...
    while (!QCD_strings_file.eof()) {
        ...
        text_stream >> new_string->mass >> new_string->m_over_sigma
            >> new_string->tau_form >> new_string->tau_0 >> new_string->eta_s_0
            >> new_string->x_perp >> new_string->y_perp >> new_string->x_pl
            >> new_string->y_pl >> new_string->x_pr >> new_string->y_pr
            >> new_string->eta_s_left >> new_string->eta_s_right
            >> new_string->y_l >> new_string->y_r >> new_string->remnant_l
        ...
        if (!text_stream.eof()) {
            ...
            music_message << "read_in_QCD_strings_and_partons: "
                          << "the format of file" << QCD_strings_filename
                          << "is wrong~";
            music_message.flush("error");
            exit(1);
        }
        ...
        new_string->tau_end_left = getStringEndTau(
            new_string->tau_0, new_string->tau_form, new_string->eta_s_0,
            new_string->eta_s_left);
        new_string->tau_end_right = getStringEndTau(
            new_string->tau_0, new_string->tau_form, new_string->eta_s_0,
            new_string->eta_s_right);
    ...
    double energyAddRadius =
        sqrt(total_energy / std::max(1., total_baryon_number) / 2760.);
    double npartAddRadius = total_baryon_number / 500.;
    double reRunAddRadius = DATA.reRunCount * 2.;
    double gridOffset =
        (DATA.gridPadding + energyAddRadius + npartAddRadius + reRunAddRadius);
    ...
    if (DATA.resetDtau) {
        ...
        DATA.delta_tau = std::min(
            DATA.delta_tau, std::min(
                                DATA.delta_x * DATA.dtaudxRatio,
    ...
    music_message << "[HydroSource] Grid info: x_size = " << DATA.x_size
                  << ", y_size = " << DATA.y_size << ", dx = " << DATA.delta_x
                  << " fm, dy = " << DATA.delta_y
...
void HydroSourceStrings::compute_norm_for_strings() {
    ...
    const int Enable_string_tilt = DATA.string_t_update;
	...
	double Pz_string_first_moment = 0.0;
        ...
        for (int ieta = 0; ieta < neta; ieta++) {
            ...
            ((eta_local - eta_c) /( 0.5 * denorm_safe)) * e_eta * sinh(y_eta);
	...
	Pz_string_first_moment *= prefactor_etas * deta;
        ...
        double Pz_string =
            (it->mass * sinh(it->y_l_i) - it->mass * sinh(it->y_l)
             + it->mass * sinh(it->y_r_i) - it->mass * sinh(it->y_r));
        Pz_string_total += Pz_string;

	double deltaPz_loss = it->mass * (sinh(it->y_r_i) - sinh(it->y_r) - sinh(it->y_l_i) + sinh(it->y_l));
	...
	it->string_t = stringTransverseShiftFrac_ ;
        ...
        it->E_remnant_norm_L =
            (E_remnant_L / std::max(E_remnant_L_norm, Util::small_eps));
        it->E_remnant_norm_R =
            (E_remnant_R / std::max(E_remnant_R_norm, Util::small_eps));
...
void HydroSourceStrings::prepare_list_for_current_tau_frame(
    ...
    for (auto &it : QCD_strings_list) {
        ...
        if (it->tau_start <= tau_local + 3. / 2. * dtau
            && std::max(it->tau_end_left, it->tau_end_right)
                   >= tau_local - dtau / 2.) {
...
void HydroSourceStrings::get_hydro_energy_source(
    ...
    if (QCD_strings_list_current_tau.size() == 0
        && QCD_strings_remnant_list_current_tau.size() == 0)
        return;
	...
	const double preFlowNorm =
        ...
        const double x_perp =
            getStringTransverseCoord(it->x_pl, it->x_pr, eta_frac, it->string_t);
        double x_dis = x - x_perp;
        if (std::abs(x_dis) > skip_dis_x) continue;

        const double y_perp =
            getStringTransverseCoord(it->y_pl, it->y_pr, eta_frac, it->string_t);
        ...
        if (tau_L > tau_0 + delta_tau) {
            eta_s_shift = acosh(
                (tau_L * tau_L + tau_0 * tau_0 - delta_tau * delta_tau)
        ...
        double eta_s_L_next =
            std::max(it->eta_s_left, it->eta_s_0 - eta_s_next_shift);
        double eta_s_R_next =
            std::min(it->eta_s_right, it->eta_s_0 + eta_s_next_shift);
        ...
        double y_string =
            (it->y_l
             + (it->y_r - it->y_l) / denorm_safe * (eta_s - it->eta_s_left));
        ...
        if (std::isnan(j_mu[0])) {
            ...
            std::cout << prefactor_etas << "  " << prefactor_prep << "  "
                      << e_local << "  " << cosh_perp << std::endl;
            std::cout << exp_tau << "  " << exp_xperp << "  " << exp_eta_s
                      << "  " << it->norm << std::endl;
	...
	const double sigma_x = it->sigma_x;
        ...
        if (it->tau_end_left >= tau - dtau / 2.
            && it->tau_end_left < tau + dtau / 2. && it->remnant_l > 0.) {
            flag_left = true;
        }

        bool flag_right = false;
        if (it->tau_end_right >= tau - dtau / 2.
            && it->tau_end_right < tau + dtau / 2. && it->remnant_r > 0.) {
            flag_right = true;
        }
                ...
                if (std::abs(x_dis) <= skip_dis_x) {
                    ...
                    if (std::abs(y_dis) <= skip_dis_x) {
                        ...
                        if (fsmear_left > 0.) {
                            ...
                            res += prefactor_etas*prefactor_prep*p_dot_u*fsmear_left;
                        }
                    }
                }
            }
                ...
                if (std::abs(x_dis) <= skip_dis_x) {
                    ...
                    if (std::abs(y_dis) <= skip_dis_x) {
                        ...
                        if (fsmear_right > 0.) {
                            ...
                            res += prefactor_etas*prefactor_prep*p_dot_u*fsmear_right;
                        }
                    }
                }
            }
...
double HydroSourceStrings::getStringTransverseCoord(
    ...
    const double etaFrac, const double string_tilt) const
