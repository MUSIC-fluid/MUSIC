...
const double MASS_PARTON           = 0.312000006;
const double MASS_PARTON_TOLERANCE = 1e-6;


//! This data structure contains a QCD string object
struct QCD_string {
    double norm;  // normalization for the string energy
    double string_t;           // strings transverse tilt 
...
class HydroSourceStrings : public HydroSourceBase {
  private:
    ...
    std::vector<std::shared_ptr<QCD_string>>
        QCD_strings_remnant_list_current_tau;
    std::vector<std::shared_ptr<QCD_string>>
        QCD_strings_baryon_list_current_tau;
  ...
  public:
    ...
    double getStringEndTau(
        const double tau0, const double tau_form, const double eta_s_0,
    ...
    double getStringTransverseCoord(
        const double xl, const double xr, const double etaFrac , const double string_tilt) const;
