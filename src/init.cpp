...
void Init::initial_IPGlasma_XY_with_pi(
    ...
    for (int ix = 0; ix < nx; ix++) {
        for (int iy = 0; iy < ny; iy++) {
            ...
            if (entropy_flag == 0) {
                epsilon =
                    (temp_profile_ed[idx] * eta_envelop_ed * DATA.sFactor
                     / hbarc);  // 1/fm^4
            } else {
                double local_sd =
                    (temp_profile_ed[idx] * DATA.sFactor * eta_envelop_ed);
                epsilon = eos.get_s2e(local_sd, rhob);
            }
            epsilon = std::max(Util::small_eps, epsilon);
...
void Init::initial_QS_with_zero_XY(
    int ieta, Fields &arenaFieldsPrev, Fields &arenaFieldsCurr) {
    double rhoq = 0.0;
    double rhos = 0.0;
    const int nx = arenaFieldsCurr.nX();
...
void Init::initial_QS_with_rhob_ratios_XY(
    int ieta, Fields &arenaFieldsPrev, Fields &arenaFieldsCurr) {
