...
int Init::InitTJb(InitData *DATA, Grid ****arena, Grid ****Lneighbor,
    ...
    if (DATA->Initial_profile == 0) {  // sangyong's initial version
        double R_A = DATA->R_A;
        double a_A = DATA->a_A;
        double a_short = DATA->a_short;
        double a_long = DATA->a_long;
        for (ix=0; ix<=DATA->nx; ix++) {
            ...
            for(iy=0; iy<=DATA->ny; iy++)
            {
                ...
                /* dUsup(4,4) -> (5,4) to use dUsup[4][m] = partial_m rhob */
...
//           (*arena)[ix][iy][ieta].pprevPimunu[0][nu][mu] = (double) 0.0;
    ...
    // this is a test scenario for testing the freeze-out surface algorithms
//      double va;
//      double test;
...
//           (*arena)[ix][iy][ieta].pprevPimunu[0][nu][mu] = (double) 0.0;
     ...
     geometry_file_rn.close();
        ...
        // sample initial distribution with a Glauber Monte-Carlo
        // and average over them to compare to average as in 1
        // sample Nu (which gives the number of nucleons at radial position r)
        // for both nuclei for now use the same nucleus for both
        int nevents = 300;
...
//           (*arena)[ix][iy][ieta].pprevPimunu[0][nu][mu] = (double) 0.0;
    ...
    } else if (DATA->Initial_profile==6) {
        //read in the profile from file - work with Ohio group
   //     double va, test;
...
//           (*arena)[ix][iy][ieta2].pprevPimunu[0][nu][mu] = (double) 0.0;
     ...
     //exit(1);
        ...
        //read in the profile from file - IPSat initial conditions
...
//           (*arena)[ix][iy][ieta2].pprevPimunu[0][nu][mu] = (double) 0.0;
     ...
     //exit(1);
        ...
        // the minimum 4 cells are good enough.
        size = DATA->size;
