...
void Init::InitArena(InitData *DATA, Grid ****arena, Grid ****Lneighbor, Grid ****Rneighbor, int size, int rank)
{
  ...
  if (DATA->Initial_profile==6 || DATA->Initial_profile==7 || DATA->Initial_profile == 8)
    {
      string dummy;
      int nx, ny, neta;
...
// 			 (*arena)[ix][iy][ieta2].pprevPimunu[0][nu][mu] = (double) 0.0;
  ...
  // the minimum 4 cells are good enough. Will work on reducing this minimum to 1 in future versions (BPS 2014/01/28)
   ...
   else if (DATA->Initial_profile==13) //read in the transverse profile from file with initial flow velocity
   {
      size = DATA->size;
