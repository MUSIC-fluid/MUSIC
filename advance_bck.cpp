...
int Advance::QuestRevert(double tau, Grid *grid_pt, int rk_flag, InitData *DATA, int size, int rank)
{
 ...
 double rho = fabs(grid_pt->Wmunu[rk_flag+1][1][1] + grid_pt->Pimunu[rk_flag+1][1][1])/grid_pt->TJb[rk_flag+1][1][1];
 if (fabs(grid_pt->Wmunu[rk_flag+1][2][2] + grid_pt->Pimunu[rk_flag+1][2][2])/grid_pt->TJb[rk_flag+1][2][2]>rho)
   rho = fabs(grid_pt->Wmunu[rk_flag+1][2][2] + grid_pt->Pimunu[rk_flag+1][2][2])/grid_pt->TJb[rk_flag+1][2][2];
 if (fabs(grid_pt->Wmunu[rk_flag+1][3][3] + grid_pt->Pimunu[rk_flag+1][3][3])/grid_pt->TJb[rk_flag+1][3][3]>rho)
   rho = fabs(grid_pt->Wmunu[rk_flag+1][3][3] + grid_pt->Pimunu[rk_flag+1][3][3])/grid_pt->TJb[rk_flag+1][3][3];
 ...
 if( (f11 < 1.1g11) || (f22 < 1.1*g22) || (f33 < 1.1*g33) )
  {
   ...
   */
   ...
   if(DATA->zero_or_stop==1)
    ...
    {
      grid_pt->pi_b[rk_flag+1] = 0.9*grid_pt->pi_b[rk_flag];
      for(mu=0; mu<4; mu++)
	{
	  for(nu=0; nu<4; nu++)
	    {
	      grid_pt->Pimunu[rk_flag+1][mu][nu] = 0.9*grid_pt->Pimunu[rk_flag+1][mu][nu];
	      grid_pt->Wmunu[rk_flag+1][mu][nu] = 0.9*grid_pt->Wmunu[rk_flag+1][mu][nu];		
	    }
	} /* mu nu */
    }
