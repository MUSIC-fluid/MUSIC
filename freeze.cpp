...
//     }
  ...
  //Compute and output the midrapidity v_n wrt pT in its own function
  Output_vn_PhiInt_midrap(DATA, number, full, iphimax, phimin, phimax);
...
//Output the phi-integrated, rapidity-averaged-around-0 yield as a function of pT
void Freeze::Output_vn_PhiInt_midrap(InitData *DATA, int number, bool full, int iphimax, double phimin, double phimax) {
	...
	const int FourierNb = 3;

	//Other useful variables
	double pt, yval, phival, phiFac, yFac,tmpIntRes;
	double yield, vn[FourierNb];
	int ystep;
	...
	for(int ipt=0;ipt<iptmax;ipt++) {
		...
		for(int iphi=0;iphi<phiQuadNodes.size();iphi++) {
			...
			//phi=(phimax-phimin)/2*u+(phimin+phimax)/2
			phival=(phimax-phimin)/2.*phiQuadNodes[iphi]+(phimin+phimax)/2.;
			...
			for(int iy=0;iy<iymax;iy++) { 
				...
				if (fabs(yval) <= midRapCut) { 
					...
					//Finally, multiply by dNdydptdphi[NY][NPT][NPHI+1] 
					//tmpIntRes+=phiFac*yFac*particleList[j].dNdydptdphi[iy][ipt][iphi];
					yield+=phiFac*yFac*particleList[j].dNdydptdphi[iy][ipt][iphi];
					for(int i=1;i<=FourierNb; i++) {
						vn[i]+=phiFac*yFac*particleList[j].dNdydptdphi[iy][ipt][iphi]*cos(i*phival);
					}
